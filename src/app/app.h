...
public:
    ...
    virtual bool keyPressed(int key, int mods) override {
        ...
        if (key == GLFW_KEY_UP) {
            controller->planner->speedForward += 0.01;
            dirty = true;
        }
        ...
        if (key == GLFW_KEY_LEFT) {
            controller->planner->turningSpeed += 0.01;
            dirty = true;
        }
...
public:
    ...
    static constexpr int kRecordMaxFrames =
        static_cast<int>(kRecordSeconds / kRecordDt + 0.5);
